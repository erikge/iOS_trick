...
class GlobalSConsEnvironment(SConsEnvironment):
	"""
	...
	class CC:
		...
		def isGCC(env):
			if 'gcc' in GlobalSConsEnvironment.CC.__getTools(env):
				return True
			return False
		isGCC = staticmethod(isGCC)

		def isMinGW(env):
			if 'mingw' in GlobalSConsEnvironment.CC.__getTools(env):
				return True
			return False
		isMinGW = staticmethod(isMinGW)

		def isMSVC(env):
			if 'msvc' in GlobalSConsEnvironment.CC.__getTools(env):
				return True
			return False
	...
	class OS:
		...
		def isWindows():
			return GlobalSConsEnvironment.OS.__platformMatch('win32')
		isWindows = staticmethod(isWindows)

		def isWindowsCE():
			#Maybe has to be seen as a compiler like inside global.h
			if GlobalSConsEnvironment.OS.isWindows():
		...
		def isLinux():
			return GlobalSConsEnvironment.OS.__platformMatch('linux')
		isLinux = staticmethod(isLinux)

		def isMacOSX():
			return GlobalSConsEnvironment.OS.__platformMatch('darwin')
		isMacOSX = staticmethod(isMacOSX)

		def isBSD():
			return GlobalSConsEnvironment.OS.__platformMatch('bsd')
		...
		def __platformMatch(platform):
			if re.search(platform, GlobalSConsEnvironment.OS.__getOS()):
				return True
			return False
	...
	def __activeWarnings(self):
		...
		self.GlobalAddCCFlags(self.currentCC.getCCWarningFlags())
	...
	def setReleaseMode(self):
		...
		self.GlobalAddCCFlags(self.currentCC.getCCFlags())
		self.GlobalAddLinkFlags(self.currentCC.getLinkFlags())
	...
	def GlobalProgram(self, programName, sources, headers = []):
		"""
	...
	def GlobalCopyToBuildDir(self, src, dest = ''):
		"""
	...
	def GlobalAutoconf(self, libraryName, outputFiles,
		...
		self.GlobalDeclareLibrary(libraryName, outputFiles)
		for file in outputFiles:
			#Gets the file path
			file = os.path.split(file)[0]
			self.GlobalAddLibPath([file])

	def GlobalDeclareMacOSXAppTemplate(self, templatePath):
		"""
	...
	def GlobalSharedLibrary(self, libraryName, sources, headers = []):
		"""
	...
	def GlobalPlugin(self, pluginName, sources, headers = []):
		"""
		Generates a plugin.
	...
	def GlobalStaticLibrary(self, libraryName, sources, headers = []):
		"""
	...
	def __installProject(self, project):
		...
		install = self.Install(self.GlobalGetRootBuildDir(), project)
	...
	def __installPDBFile(self):
		...
		if self.CC.isMSVC(self) and (self.isDebugMode() or self.isReleaseModeWithSymbols()):
			install = self.Install(self.GlobalGetRootBuildDir(), self.__getProjectName() + '.pdb')
			self.Alias(self.__getProjectName(), install)
	...
	def GlobalAddCCFlags(self, flags):
		"""
	...
	def GlobalAddLinkFlags(self, flags):
		"""
	...
	def GlobalAddLibPath(self, paths):
		"""
	...
	def GlobalAddDefines(self, defines):
		"""
	...
	def GlobalAddIncludePath(self, paths):
		"""
	...
	def GlobalDeclareLibrary(self, projectName, files):
		"""
		Declares an internal library and add it to the dictionnary of libraries.
		...
		for file in files:
			install = self.Install(self.GlobalGetRootBuildDir(), file)
			self.Alias(self.__getProjectName(), install)

	def GlobalDeclareSystemLibrary(self, projectName):
		"""
	...
	def GlobalGetRootBuildDir(self):
		"""
	...
	def __getBuildDirAbsPath(self, subpath):
		...
		return os.path.abspath(os.path.join(self.GlobalGetRootBuildDir(), subpath))
	...
	def __linkWithLibrary(self, libEnv):
		...
		if libEnv.__isSystemLibrary:
			self.GlobalAddLibPath(libEnv.LibPath.getLibPath(libEnv))
			self.__addLibs(libEnv.Libs.getLibs(libEnv))
		else:
			self.GlobalAddLibPath([self.__getBuildDirAbsPath(libEnv.__getBuildPath())])
	...
	def __useLibrary(self, libEnv):
		...
		if libEnv.__isSystemLibrary:
			self.GlobalAddIncludePath(libEnv.IncludePath.getIncludePath(libEnv))
			self.GlobalAddLibPath(libEnv.LibPath.getLibPath(libEnv))
			self.__addLibs(libEnv.Libs.getLibs(libEnv))
			self.GlobalAddCCFlags(libEnv.CCFlags.getCCFlags(libEnv))
			self.GlobalAddLinkFlags(libEnv.LinkFlags.getLinkFlags(libEnv))
			self.GlobalAddDefines(libEnv.Defines.getDefines(libEnv))
		else:
			tmp1 = []
	...
	def GlobalUseLibraries(self, libs):
		"""
	...
	def GlobalCompileWindowsResource(self, rcFile):
		"""
	...
	def GlobalCompileQt4Resource(self, qrcFile):
		"""
	...
	def GlobalCreateFile(self, filename, fileTemplate, fileData):
		"""
...
#FIXME ugly?
GlobalSConsEnvironment._globalEnv = None
...
def getGlobalEnvironment(*args, **kwargs):
	...
	#Lazy initialization
	if not GlobalSConsEnvironment._globalEnv:
		#Singleton WengoSConsEnvironment
		GlobalSConsEnvironment._globalEnv = GlobalSConsEnvironment(*args, **kwargs)

		#Saves the root build directory path
		GlobalSConsEnvironment._globalEnv['ROOT_BUILD_DIR'] = GlobalSConsEnvironment._globalEnv.Dir('.').abspath

	return GlobalSConsEnvironment._globalEnv

def GlobalGetEnvironment():
	"""
...
def GlobalGetQt4Environment():
	...
	return GlobalGetQt3Environment()

def GlobalGetRootBuildDir():
	"""
	@see WengoSConsEnvironment.WengoGetRootBuildDir()
	"""

	env = getGlobalEnvironment()
	return env.GlobalGetRootBuildDir()

def GlobalAddCCFlags(flags):
	"""
	...
	env = getGlobalEnvironment()
	env.GlobalAddCCFlags(flags)

def GlobalAddDefines(defines):
	"""
	...
	env.GlobalAddDefines(defines)

def GlobalAddIncludePath(paths):
	"""
	...
	env = environments.getGlobalEnvironment()
	env.GlobalAddIncludePath(paths)
...
def GlobalOSPosix():
	...
	if GlobalOSLinux() or GlobalOSMacOSX() or GlobalOSBSD():
		return True
	else:
		return False
...
def GlobalSetReleaseModeWithSymbols():
	"""
...
def GlobalSetVariable(variable, value):
	"""
...
def GlobalGetVariable(variable):
	"""
...
def GlobalAddConsoleArguments(args):
	"""
...
def GlobalGetConsoleArgument(arg):
	"""
...
def GlobalShowWindowsConsole(show):
	"""
	...
	if GlobalCCMSVC():
		if show:
			...
			env.GlobalAddLinkFlags(['/subsystem:console'])
		else:
			env.LinkFlags.removeLinkFlags(env, ['/subsystem:console'])
			env.GlobalAddLinkFlags(['/subsystem:windows'])

def GlobalUseLibraries(libs):
	"""
	...
	env = getGlobalEnvironment()
	env.GlobalUseLibraries(libs)

def GlobalGetSubversionRevision():
	"""
